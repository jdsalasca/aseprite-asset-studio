...
import type { AssetGateway, AssetLibraryAuditView, AssetLibraryPresetCompositionView, AssetLibrarySearchView, AssetLibrarySummaryView, AssetLibraryVariantPackView, AssetManifestAuditView, AssetRecipeExecutionView, AssetSceneAnimationCompositionView, AssetSceneBundleView, AssetSceneCompositionView, AssetScenePlanView, AssetSceneRecommendationView, HealthResponse, RuntimeConfig, StoredAsset, ToolDescriptor, ToolRuntimeStatus } from "../src/domain/contracts.js";
...
class FakeGateway implements AssetGateway {
  ...
  public async callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
    ...
    if (name === "generate_sprite_silhouette") return { content: [{ text: JSON.stringify({ operation: name, output: args.output_filename, frames: 1, format: args.format, deterministic: true, sourcePreserved: true }) }] };
    ...
    if (name === "audit_asset_library") return { content: [{ text: JSON.stringify({ operation: name, libraryVersion: "catalog-v1", totalItems: 339, totalCategories: 24, totalPresets: 12, totalFolders: 339, readmePaths: 339, previewPaths: 339, spritePaths: 339, animationPaths: 339, valid: true, violations: [], deterministic: true, sourcePreserved: true } satisfies AssetLibraryAuditView) }] };
    ...
    if (name === "recommend_asset_scene") return { content: [{ text: JSON.stringify({ operation: name, libraryVersion: "catalog-v1", input: { prompt: String(args.prompt), limit: Number(args.limit), seed: Number(args.seed) }, recommendations: [{ assetId: "ocean", title: "Ocean Waves", category: "coast", kind: "scene", score: 21, reasons: ["prompt:water", "variant:water_reflection"], tags: ["water"], variants: ["water_reflection"] }], suggestedItemIds: ["ocean"], coveredKinds: ["scene"], coveredTags: ["water"], coveredVariants: ["water_reflection"], deterministic: true, sourcePreserved: true } satisfies AssetSceneRecommendationView) }] };
    if (name === "build_scene_bundle") return { content: [{ text: JSON.stringify({ operation: name, outputPrefix: String(args.output_prefix), itemIds: args.item_ids as string[], static: { operation: "compose_asset_scene", output: "out/bundle/scene.png", manifest: "out/bundle/scene.json", libraryVersion: "v1", itemIds: args.item_ids as string[], width: 64, height: 48, padding: 2, layers: [], deterministic: true, sourcePreserved: true }, animation: { operation: "compose_asset_scene_animation", output: "out/bundle/scene.gif", manifest: "out/bundle/scene-animation.json", libraryVersion: "v1", itemIds: args.item_ids as string[], width: 64, height: 48, padding: 2, frames: 8, delayMs: 90, frameLayers: [], deterministic: true, sourcePreserved: true }, deterministic: true, sourcePreserved: true } satisfies AssetSceneBundleView) }] };
...
describe("AssetStudioService enhancement use cases", () => {
  ...
  it("maps smoke controls to the shared MCP environmental generator", async () => {
    const gateway = new FakeGateway();
    const result = await new AssetStudioService(gateway).applySpriteEffect("smoke", "scene.png", "scene-smoke.gif", { frames: 6, seed: 41, density: 0.75, drift: 0.2, rise: 0.8, color: "#8A91A8", delay_ms: 100 });
    expect(result).toMatchObject({ operation: "generate_smoke_overlay", output: "scene-smoke.gif", format: "gif" });
    expect(gateway.calls.at(-1)).toMatchObject({ name: "generate_smoke_overlay", args: { input_filename: "scene.png", output_filename: "scene-smoke.gif", frames: 6, seed: 41, density: 0.75, drift: 0.2, rise: 0.8, color: "#8A91A8", delay_ms: 100, format: "gif" } });
  });

  it("maps fire controls to the shared MCP environmental generator", async () => {
    const gateway = new FakeGateway();
    const result = await new AssetStudioService(gateway).applySpriteEffect("fire", "scene.png", "scene-fire.gif", { frames: 6, seed: 53, intensity: 0.82, flicker: 0.7, color: "#FFD65A", delay_ms: 100 });
    expect(result).toMatchObject({ operation: "generate_fire_overlay", output: "scene-fire.gif", format: "gif" });
    expect(gateway.calls.at(-1)).toMatchObject({ name: "generate_fire_overlay", args: { input_filename: "scene.png", output_filename: "scene-fire.gif", frames: 6, seed: 53, intensity: 0.82, flicker: 0.7, color: "#FFD65A", delay_ms: 100, format: "gif" } });
  });

  it("maps lightning controls to the shared MCP environmental generator", async () => {
    const gateway = new FakeGateway();
    const result = await new AssetStudioService(gateway).applySpriteEffect("lightning", "scene.png", "scene-lightning.gif", { frames: 6, seed: 67, intensity: 0.72, flash: 0.8, color: "#D8F3FF", delay_ms: 100 });
    expect(result).toMatchObject({ operation: "generate_lightning_overlay", output: "scene-lightning.gif", format: "gif" });
    expect(gateway.calls.at(-1)).toMatchObject({ name: "generate_lightning_overlay", args: { input_filename: "scene.png", output_filename: "scene-lightning.gif", frames: 6, seed: 67, intensity: 0.72, flash: 0.8, color: "#D8F3FF", delay_ms: 100, format: "gif" } });
  });

  it("maps wave controls to the shared MCP environmental generator", async () => {
    const gateway = new FakeGateway();
  ...
  it("maps dust controls to the shared MCP environmental generator", async () => {
    const gateway = new FakeGateway();
    const result = await new AssetStudioService(gateway).applySpriteEffect("dust", "hero.png", "hero-dust.gif", { frames: 5, seed: 97, density: 0.72, drift: 0.25, rise: 0.5, color: "#C79A68", delay_ms: 90 });
    expect(result).toMatchObject({ operation: "generate_dust_overlay", output: "hero-dust.gif", format: "gif" });
    expect(gateway.calls.at(-1)).toMatchObject({ name: "generate_dust_overlay", args: { input_filename: "hero.png", output_filename: "hero-dust.gif", frames: 5, seed: 97, density: 0.72, drift: 0.25, rise: 0.5, color: "#C79A68", delay_ms: 90, format: "gif" } });
  });

  it("maps leaf-fall controls to the shared MCP environmental generator", async () => {
    const gateway = new FakeGateway();
    const result = await new AssetStudioService(gateway).applySpriteEffect("leaf_fall", "tree.png", "tree-leaves.gif", { frames: 5, seed: 103, density: 0.68, wind: -0.2, color: "#D97732", delay_ms: 85 });
    expect(result).toMatchObject({ operation: "generate_leaf_fall_overlay", output: "tree-leaves.gif", format: "gif" });
    expect(gateway.calls.at(-1)).toMatchObject({ name: "generate_leaf_fall_overlay", args: { input_filename: "tree.png", output_filename: "tree-leaves.gif", frames: 5, seed: 103, density: 0.68, wind: -0.2, color: "#D97732", delay_ms: 85, format: "gif" } });
  });

  it("maps water-ripple controls to the shared MCP environmental generator", async () => {
    const gateway = new FakeGateway();
    const result = await new AssetStudioService(gateway).applySpriteEffect("ripple", "pond.png", "pond-ripple.gif", { frames: 5, seed: 109, density: 0.72, amplitude: 3, color: "#BDEBFF", delay_ms: 90 });
    expect(result).toMatchObject({ operation: "generate_water_ripple_overlay", output: "pond-ripple.gif", format: "gif" });
    expect(gateway.calls.at(-1)).toMatchObject({ name: "generate_water_ripple_overlay", args: { input_filename: "pond.png", output_filename: "pond-ripple.gif", frames: 5, seed: 109, density: 0.72, amplitude: 3, color: "#BDEBFF", delay_ms: 90, format: "gif" } });
  });
  ...
  it("maps isolated-pixel cleanup to the shared MCP effects service", async () => {
    const gateway = new FakeGateway();
    const result = await new AssetStudioService(gateway).applySpriteEffect("cleanup", "source.gif", "source-cleanup.gif", { min_neighbors: 1, iterations: 2 });
    expect(result).toMatchObject({ operation: "cleanup_isolated_pixels", output: "source-cleanup.gif", format: "gif", sourcePreserved: true });
    expect(gateway.calls.at(-1)).toMatchObject({ name: "cleanup_isolated_pixels", args: { input_filename: "source.gif", output_filename: "source-cleanup.gif", min_neighbors: 1, iterations: 2, format: "gif" } });
  });

  it("maps sprite glow to the shared MCP effects service", async () => {
    const gateway = new FakeGateway();
  ...
  it("maps sprite ambient occlusion to the shared MCP effects service", async () => {
    const gateway = new FakeGateway();
  ...
  it("maps seeded sprite grain to the shared MCP effects service", async () => {
    const gateway = new FakeGateway();
    const result = await new AssetStudioService(gateway).applySpriteEffect("grain", "source.gif", "source-grain.gif", { seed: 17, intensity: 0.8, scale: 2 });
    expect(result).toMatchObject({ operation: "apply_sprite_grain", output: "source-grain.gif", format: "gif", sourcePreserved: true });
    expect(gateway.calls.at(-1)).toMatchObject({ name: "apply_sprite_grain", args: { input_filename: "source.gif", output_filename: "source-grain.gif", seed: 17, intensity: 0.8, scale: 2, format: "gif" } });
  });

  it("maps Bayer sprite dithering to the shared MCP effects service", async () => {
    const gateway = new FakeGateway();
  ...
  it("maps sprite silhouette controls to the shared MCP effects service", async () => {
    const gateway = new FakeGateway();
    const result = await new AssetStudioService(gateway).applySpriteEffect("silhouette", "source.gif", "source-silhouette.gif", { color: "#08111F", opacity: 0.6 });
    expect(result).toMatchObject({ operation: "generate_sprite_silhouette", output: "source-silhouette.gif", format: "gif", sourcePreserved: true });
    expect(gateway.calls.at(-1)).toMatchObject({ name: "generate_sprite_silhouette", args: { input_filename: "source.gif", output_filename: "source-silhouette.gif", color: "#08111F", opacity: 0.6, format: "gif" } });
  });
  ...
  it("requests explainable scene recommendations through the shared MCP gateway", async () => {
    const gateway = new FakeGateway();
    const result = await new AssetStudioService(gateway).recommendAssetScene({ prompt: "coastal water", requiredVariants: ["water_reflection"], limit: 4, seed: 7 });
    expect(result).toMatchObject({ operation: "recommend_asset_scene", suggestedItemIds: ["ocean"], coveredVariants: ["water_reflection"] });
    expect(gateway.calls[0]).toMatchObject({ name: "recommend_asset_scene", args: { prompt: "coastal water", required_variants: ["water_reflection"], limit: 4, seed: 7 } });
  });

  it("builds a static and animated scene bundle through one MCP request", async () => {
    const gateway = new FakeGateway();
    const result = await new AssetStudioService(gateway).buildSceneBundle({ itemIds: ["oak"], outputPrefix: "out/bundle", width: 64, height: 48, padding: 2, frames: 8, delayMs: 90 });
    expect(result).toMatchObject({ operation: "build_scene_bundle", outputPrefix: "out/bundle", static: { operation: "compose_asset_scene" }, animation: { operation: "compose_asset_scene_animation" } });
    expect(gateway.calls[0]).toMatchObject({ name: "build_scene_bundle", args: { item_ids: ["oak"], output_prefix: "out/bundle", frames: 8 } });
  });
