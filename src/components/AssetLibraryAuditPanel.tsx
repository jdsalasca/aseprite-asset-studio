...
export function AssetLibraryAuditPanel({ busy, online, result, onAudit }: AssetLibraryAuditPanelProps) {
  return <PixelPanel title="LIBRARY AUDIT" accent={result?.valid === false ? "amber" : "cyan"}>
    ...
    {result ? <PixelLibraryAudit totalItems={result.totalItems} totalCategories={result.totalCategories} totalPresets={result.totalPresets} totalFolders={result.totalFolders} readmePaths={result.readmePaths} previewPaths={result.previewPaths} spritePaths={result.spritePaths} animationPaths={result.animationPaths} valid={result.valid} violations={result.violations} /> : null}
  </PixelPanel>;
}
