...
const OPTIONS: Array<{ value: AssetVariantKind; label: string }> = [
  { value: "rain", label: "RAIN" }, { value: "fire", label: "FIRE" }, { value: "earthquake", label: "EARTHQUAKE" }, { value: "birds", label: "BIRDS" }, { value: "night", label: "NIGHT" }, { value: "day_night", label: "DAY / NIGHT" }, { value: "walk", label: "WALK" }, { value: "water_reflection", label: "WATER REFLECTION" }, { value: "water_caustics", label: "WATER CAUSTICS" },
];
...
export function VariantPackPanel({ busy, online, assetName, onGenerate }: VariantPackPanelProps) {
  ...
  function generate(): void {
    ...
    if (selected.length === 0 || selected.length > 9 || !Number.isInteger(parsedFrames) || parsedFrames < 2 || parsedFrames > 24 || !Number.isInteger(parsedSeed)) return;
    onGenerate(selected, parsedFrames, parsedSeed);
  }
  return <PixelPanel title="ENVIRONMENT VARIANT PACK" accent="amber">
    <p className="muted">Crea salidas separadas de lluvia, fuego, terremoto, pájaros, noche, movimiento y agua para <strong>{assetName}</strong>.</p>
