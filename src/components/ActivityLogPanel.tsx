...
export function ActivityLogPanel({ entries }: { entries: OperationLogEntry[] }) {
  ...
  return <PixelPanel title="ACTIVITY LOG"><PixelField label="FILTER LOG" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="failure, tool name..." /><div className="activity-log"><PixelLogViewer entries={filtered.slice().reverse().map((entry) => ({ id: `${entry.correlationId}-${entry.outcome}`, timestamp: entry.timestamp, title: `${entry.outcome.toUpperCase()} · ${entry.operation}`, status: entry.outcome, detail: entry.error ?? `${entry.durationMs}ms · ${entry.correlationId}` }))} /></div></PixelPanel>;
}
