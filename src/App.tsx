...
export default function App() {
  ...
  async function inspect() { if (!assetPath || status.state !== "online") return; setBusy(true); setNotice("Inspeccionando referencia..."); try { const result = await service.callTool("inspect_reference", { filename: assetPath }) as { content?: Array<{ text?: string }> }; setNotice(`Referencia inspeccionada: ${result.content?.[0]?.text ?? "resultado recibido"}`); } catch (error) { setNotice(error instanceof Error ? error.message : String(error)); } finally { setBusy(false); } }
