...
export default function App() {
  ...
  async function applyPlan() { if (!assetPath || status.state !== "online") return; const outputFilename = assetPath.replace(/\.[^./\\]+$/, "-enhanced.png"); setBusy(true); setNotice("Aplicando plan en un archivo separado..."); try { const result = await service.applyEnhancementPlan(assetPath, outputFilename); setEnhancedPreviewUrl(service.assetPreviewUrl(result.outputFilename)); setNotice(`Salida creada: ${result.outputFilename} · ${result.passesApplied.length} pasadas · fuente preservada.`); } catch (error) { setNotice(error instanceof Error ? error.message : String(error)); } finally { setBusy(false); } }
