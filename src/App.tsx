...
export default function App() {
  ...
  async function inspect() { if (!assetPath || status.state !== "online") return; setBusy(true); setNotice("Inspeccionando referencia y calculando plan..."); try { const result = await service.callTool("inspect_reference", { filename: assetPath }) as { content?: Array<{ text?: string }> }; const planResult = await service.callTool("suggest_enhancement_plan", { filename: assetPath, goals: ["cleanup", "terrain_grain", "water_flow", "directional_lighting", "particles"] }) as { content?: Array<{ text?: string }> }; const planText = planResult.content?.[0]?.text; if (planText) setPlan(JSON.parse(planText) as EnhancementPlanView); setNotice(`Referencia inspeccionada: ${result.content?.[0]?.text ?? "resultado recibido"}`); } catch (error) { setNotice(error instanceof Error ? error.message : String(error)); } finally { setBusy(false); } }
