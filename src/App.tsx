...
import { AssetLibraryVariantPackPanel } from "./components/AssetLibraryVariantPackPanel.js";
import { AssetManifestAuditPanel } from "./components/AssetManifestAuditPanel.js";
import { AssetSceneRecommendationPanel } from "./components/AssetSceneRecommendationPanel.js";
import { AssetSceneBundlePanel } from "./components/AssetSceneBundlePanel.js";
...
import { EnhancementBatchPanel } from "./components/EnhancementBatchPanel.js";
import { AnimationAuditPanel } from "./components/AnimationAuditPanel.js";
import { SpriteNormalizationPanel } from "./components/SpriteNormalizationPanel.js";
import { AnimationSheetPanel } from "./components/AnimationSheetPanel.js";
import { SpriteGeometryPanel } from "./components/SpriteGeometryPanel.js";
import { SpriteHitboxPanel } from "./components/SpriteHitboxPanel.js";
import { SpriteRuntimeBundlePanel } from "./components/SpriteRuntimeBundlePanel.js";
import { SpriteAnchorsPanel } from "./components/SpriteAnchorsPanel.js";
  ...
  return <main className="studio-shell">
    ...
    <div className="studio-grid">
      <div className="studio-main">
        ...
        {assetPath ? <EnhancementBatchPanel busy={busy} online={status.state === "online"} result={enhancementBatch} onApply={() => void applyEnhancementBatch()} /> : null}
        ...
        {assetPath ? <SpriteNormalizationPanel busy={busy} online={status.state === "online"} assetName={assetName} result={normalizedSprite} onNormalize={(padding, pivot) => void normalizeSprite(padding, pivot)} /> : null}
        {assetPath ? <AnimationSheetPanel busy={busy} online={status.state === "online"} assetName={assetName} previewUrl={enhancedPreviewUrl} result={animationSheet} onBuild={(columns, padding) => void buildAnimationSheet(columns, padding)} /> : null}
        {assetPath ? <SpriteGeometryPanel busy={busy} online={status.state === "online"} assetName={assetName} result={spriteGeometry} onInspect={() => void inspectSpriteGeometry()} /> : null}
        {assetPath ? <SpriteHitboxPanel busy={busy} online={status.state === "online"} assetName={assetName} result={spriteHitboxes} onGenerate={(mode, padding) => void generateSpriteHitboxes(mode, padding)} /> : null}
        {assetPath ? <SpriteRuntimeBundlePanel busy={busy} online={status.state === "online"} assetName={assetName} result={spriteRuntimeBundle} onBuild={(mode, columns) => void buildSpriteRuntimeBundle(mode, columns)} /> : null}
        {assetPath ? <SpriteAnchorsPanel busy={busy} online={status.state === "online"} assetName={assetName} result={spriteAnchors} onGenerate={() => void generateSpriteAnchors()} /> : null}
        ...
        <AssetSceneComposerPanel busy={busy} online={status.state === "online"} result={assetSceneComposition} animationResult={assetSceneAnimationComposition} previewUrl={variantPreviewUrl} onCompose={(input) => void composeAssetScene(input)} onComposeAnimation={(input) => void composeAssetSceneAnimation(input)} />
        <AssetLibraryVariantPackPanel busy={busy} online={status.state === "online"} result={assetLibraryVariantPack} previewUrl={variantPreviewUrl} onGenerate={(input) => void generateLibraryVariantPack(input)} />
        <AssetManifestAuditPanel busy={busy} online={status.state === "online"} result={assetManifestAudit} onAudit={(manifestFilename) => void auditAssetManifest(manifestFilename)} />
        <AssetSceneRecommendationPanel busy={busy} online={status.state === "online"} result={assetSceneRecommendation} onRecommend={(input) => void recommendAssetScene(input)} />
        <AssetSceneBundlePanel busy={busy} online={status.state === "online"} result={assetSceneBundle} previewUrl={variantPreviewUrl} onBuild={(input) => void buildSceneBundle(input)} />
