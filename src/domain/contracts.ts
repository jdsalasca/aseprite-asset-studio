...
export type SpriteEffectKind = "outline" | "color_grade" | "shadow" | "particles" | "normal_map" | "rain" | "fog" | "snow" | "smoke" | "fire" | "lightning" | "waves" | "water_spray" | "dust" | "leaf_fall" | "ripple" | "motion" | "wind_sway" | "upscale" | "seamless" | "reflection" | "caustics" | "day_night" | "background" | "cleanup" | "glow" | "silhouette" | "rim_light" | "ambient_occlusion" | "specular_highlight" | "color_ramp" | "grain" | "dither" | "color_temperature";
...
export interface AssetLibraryItemView { id: string; title: string; category: string; folder: string; kind: "sprite" | "tileset" | "scene" | "effect" | "character" | "prop"; description: string; tags: string[]; variants: string[]; formats: Array<"png" | "gif" | "svg" | "json">; readmePath: string; previewPath: string; spritePath: string; animationPath?: string; deterministic: true; }
...
export interface AssetManifestAuditView { operation: "audit_asset_manifest"; manifest: string; artifacts: Array<{ filename: string; status: "ok" | "missing" | "empty"; sizeBytes: number; format: string; sha256: string | null }>; totalArtifacts: number; missingArtifacts: number; emptyArtifacts: number; valid: boolean; deterministic: true; sourcePreserved: true; }
export interface AssetSceneRecommendationView { operation: "recommend_asset_scene"; libraryVersion: string; input: { prompt?: string; category?: string; requiredKinds?: string[]; requiredTags?: string[]; requiredVariants?: string[]; limit: number; seed: number }; recommendations: Array<{ assetId: string; title: string; category: string; kind: string; score: number; reasons: string[]; tags: string[]; variants: string[] }>; suggestedItemIds: string[]; coveredKinds: string[]; coveredTags: string[]; coveredVariants: string[]; deterministic: true; sourcePreserved: true; }
export interface AssetSceneBundleView { operation: "build_scene_bundle"; outputPrefix: string; itemIds: string[]; static: AssetSceneCompositionView; animation: AssetSceneAnimationCompositionView; deterministic: true; sourcePreserved: true; }
...
export interface AssetLibraryVariantPackView { operation: "generate_library_variant_pack"; manifest: string; libraryVersion: string; itemIds: string[]; outputPrefix: string; variants: AssetVariantKind[]; frames: number; seed: number; assets: Array<{ assetId: string; title: string; outputPrefix: string; artifacts: AssetVariantArtifactView[] }>; deterministic: true; sourcePreserved: true; }
