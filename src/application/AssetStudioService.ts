...
export class AssetStudioService {
  ...
  public async applySpriteEffect(kind: SpriteEffectKind, filename: string, outputFilename: string, options: Record<string, number | string | boolean> = {}): Promise<SpriteEffectView> {
    return this.trace(`apply_${kind}`, async () => {
      const operation = kind === "outline" ? "apply_pixel_outline" : kind === "background" ? "remove_background" : kind === "cleanup" ? "cleanup_isolated_pixels" : kind === "glow" ? "generate_sprite_glow" : kind === "silhouette" ? "generate_sprite_silhouette" : kind === "rim_light" ? "apply_sprite_rim_light" : kind === "ambient_occlusion" ? "apply_sprite_ambient_occlusion" : kind === "specular_highlight" ? "apply_sprite_specular_highlight" : kind === "color_ramp" ? "apply_sprite_color_ramp" : kind === "grain" ? "apply_sprite_grain" : kind === "dither" ? "apply_sprite_dither" : kind === "color_temperature" ? "apply_sprite_color_temperature" : kind === "color_grade" ? "apply_color_grade" : kind === "shadow" ? "generate_sprite_shadow" : kind === "particles" ? "generate_particle_burst" : kind === "normal_map" ? "generate_normal_map" : kind === "rain" ? "generate_rain_overlay" : kind === "fog" ? "generate_fog_overlay" : kind === "snow" ? "generate_snow_overlay" : kind === "smoke" ? "generate_smoke_overlay" : kind === "fire" ? "generate_fire_overlay" : kind === "lightning" ? "generate_lightning_overlay" : kind === "waves" ? "generate_wave_overlay" : kind === "water_spray" ? "generate_water_spray" : kind === "dust" ? "generate_dust_overlay" : kind === "leaf_fall" ? "generate_leaf_fall_overlay" : kind === "ripple" ? "generate_water_ripple_overlay" : kind === "motion" ? "generate_motion_pack" : kind === "wind_sway" ? "generate_wind_sway" : kind === "upscale" ? "upscale_pixel_art" : kind === "reflection" ? "generate_water_reflection" : kind === "caustics" ? "generate_water_caustics" : kind === "day_night" ? "generate_day_night_cycle" : "generate_seamless_texture";
      const args: Record<string, unknown> = kind === "particles"
        ...
        : kind === "background"
          ...
          : kind === "silhouette"
            ? { input_filename: filename, output_filename: outputFilename, format: /\.gif$/i.test(filename) ? "gif" : "png", color: options.color ?? "#08111F", opacity: options.opacity ?? 1 }
          : kind === "rim_light"
          ...
          : kind === "color_temperature"
            ? { input_filename: filename, output_filename: outputFilename, format: /\.gif$/i.test(filename) ? "gif" : "png", temperature: options.temperature ?? 0, intensity: options.intensity ?? 0.7 }
          : kind === "rain"
          ...
          : kind === "fog"
            ? { input_filename: filename, output_filename: outputFilename, format: "gif", seed: options.seed ?? 1, density: options.density ?? 0.55, drift: options.drift ?? 0, color: options.color ?? "#DDEBFF", frames: options.frames ?? 8, delay_ms: options.delay_ms ?? 90 }
          : kind === "snow"
            ? { input_filename: filename, output_filename: outputFilename, format: "gif", seed: options.seed ?? 1, density: options.density ?? 0.55, wind: options.wind ?? 0, color: options.color ?? "#F5FBFF", frames: options.frames ?? 8, delay_ms: options.delay_ms ?? 90 }
          : kind === "smoke"
            ? { input_filename: filename, output_filename: outputFilename, format: "gif", seed: options.seed ?? 1, density: options.density ?? 0.55, drift: options.drift ?? 0, rise: options.rise ?? 0.65, color: options.color ?? "#8A91A8", frames: options.frames ?? 8, delay_ms: options.delay_ms ?? 90 }
          : kind === "fire"
            ? { input_filename: filename, output_filename: outputFilename, format: "gif", seed: options.seed ?? 1, intensity: options.intensity ?? 0.65, flicker: options.flicker ?? 0.55, color: options.color ?? "#FFD65A", frames: options.frames ?? 8, delay_ms: options.delay_ms ?? 90 }
          : kind === "lightning"
            ? { input_filename: filename, output_filename: outputFilename, format: "gif", seed: options.seed ?? 1, intensity: options.intensity ?? 0.65, flash: options.flash ?? 0.65, color: options.color ?? "#D8F3FF", frames: options.frames ?? 8, delay_ms: options.delay_ms ?? 90 }
          : kind === "waves"
            ? { input_filename: filename, output_filename: outputFilename, format: "gif", seed: options.seed ?? 1, density: options.density ?? 0.65, amplitude: options.amplitude ?? 2, color: options.color ?? "#E6FAFF", frames: options.frames ?? 8, delay_ms: options.delay_ms ?? 90 }
          : kind === "water_spray"
            ? { input_filename: filename, output_filename: outputFilename, format: "gif", seed: options.seed ?? 1, density: options.density ?? 0.6, drift: options.drift ?? 0, color: options.color ?? "#F4FDFF", frames: options.frames ?? 8, delay_ms: options.delay_ms ?? 90 }
          : kind === "dust"
            ? { input_filename: filename, output_filename: outputFilename, format: "gif", seed: options.seed ?? 1, density: options.density ?? 0.6, drift: options.drift ?? 0, rise: options.rise ?? 0.5, color: options.color ?? "#C79A68", frames: options.frames ?? 8, delay_ms: options.delay_ms ?? 90 }
          : kind === "leaf_fall"
            ? { input_filename: filename, output_filename: outputFilename, format: "gif", seed: options.seed ?? 1, density: options.density ?? 0.6, wind: options.wind ?? 0, color: options.color ?? "#D97732", frames: options.frames ?? 8, delay_ms: options.delay_ms ?? 90 }
          : kind === "ripple"
            ? { input_filename: filename, output_filename: outputFilename, format: "gif", seed: options.seed ?? 1, density: options.density ?? 0.6, amplitude: options.amplitude ?? 2, color: options.color ?? "#BDEBFF", frames: options.frames ?? 8, delay_ms: options.delay_ms ?? 90 }
          : kind === "motion"
            ...
            : kind === "wind_sway"
              ? { input_filename: filename, output_filename: outputFilename, format: "gif", frames: options.frames ?? 8, seed: options.seed ?? 1, amplitude: options.amplitude ?? 2, direction: options.direction ?? "right", delay_ms: options.delay_ms ?? 90 }
            : kind === "upscale"
